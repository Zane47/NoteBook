...
![image-20211122101342894](C:\github\NoteBook\img\MySQL-索引\image-20211122101342894.png)

一共8条记录, 每一条记录很大, 每个block只能存2个, 所以就需要4个block. 如果我们要查询的数据在最后一个block(name = 'Green'), 就需要遍历所有的blocks, 4IOs

如果我们使用index索引, 首先在索引中找到数据, 再到具体的block中去找, 2IOs

![image-20211122101433148](C:\github\NoteBook\img\MySQL-索引\image-20211122101433148.png)
...
- 对于非常小的表, 大部分情况下简单的全表扫描比建立索引更高效；对于中到大型表索引都是非常有效的；但是特大型表的话, 建立和维护索引的代价将会随之增长，不适合建索引, 这种情况下，需要用到一种技术可以直接区分出需要查询的一组数据，而不是一条记录一条记录地匹配，例如可以使用分区技术。
...
## ref
https://github.com/Snailclimb/JavaGuide/edit/main/docs/database/mysql/mysql-index.md
